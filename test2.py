...
def save_to_postgres(value):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                f"INSERT INTO test (timestamp, value, tag_name, source) VALUES (%s, %s, %s, %s, %s)",
...
def read_modbus_value():
    ...
    try:
        with ModbusTcpClient(MODBUS_IP, port=MODBUS_PORT) as client:
            ...
            # raw_value = response.registers[0] + response.registers[1]
            raw_value = struct.pack('>HH', response.registers[1], response.registers[0])
            float_value = struct.unpack('>f', raw_value)[0]
            print(float_value)  # Результат: 83.94499969482422 (более осмысленное значение)
            # Пример преобразования (уточните формат данных для вашего устройства)
            value = float_value / 1.0  # Пример: если значение передается как целое ×10
            return value
